...
5. 检查 docs/modules/ 分支一致性
   - 运行: git diff main -- docs/modules/
   - 如有差异，提醒用户审查后再继续
```
...
**目的：** 将澄清问题中收集的需求整理为正式文档

**文档路径：** `docs/specs/feature_{模块}_{功能}_{日期}/requirements.md`

**使用模板：** `docs/templates/requirements-template.md`
...
**提交操作：**
```bash
git add docs/specs/feature_{模块}_{功能}_{日期}/requirements.md
git commit -m "docs: add requirements doc for {功能}"
```
...
**文档位置：** `docs/specs/feature_{模块}_{功能}_{日期}/design.md`
...
1. 使用 Write 工具创建文档
2. 使用 Bash 提交到 Git:
   git add docs/specs/feature_{模块}_{功能}_{日期}/design.md
   git commit -m "docs: add design for <功能>"
```
...
使用 Task 工具:
  ...
  prompt: |
    ...
    **Spec to review:** docs/specs/feature_{模块}_{功能}_{日期}/design.md
...
**操作：**
```
设计文档已编写并提交到 docs/specs/feature_{模块}_{功能}_{日期}/design.md
...
| 用户响应 | 处理方式 |
|----------|----------|
| 批准 | 进入 Step 11，调用 writing-plans |
| 请求修改 | 修改后重新运行 Step 9 审查循环 |
| 提出问题 | 回答问题后确认是否需要修改 |
...
### Q1: docs/modules/ 分支检查发现差异怎么办？

**处理：**
```
1. 停止当前流程
2. 告知用户：检测到 docs/modules/ 与 main 分支不一致
3. 请用户审查差异内容：
   git diff main -- docs/modules/
4. 用户确认后再继续 brainstorming
```
